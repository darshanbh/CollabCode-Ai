...
function EditorRoom() {
  ...
  useEffect(() => {
    ...
    const fetchRoom = async () => {
      try {
        ...
        if (room.codeContent) setCode(room.codeContent);
        if (room.language) setLanguage(room.language);
        if (room.examQuestion) setExamQuestion(room.examQuestion);
  ...
  const handleQuestionChange = (e) => {
    ...
    setExamQuestion(newQ);
    setQuestionSaveStatus('Saving...');
    clearTimeout(questionTimeoutRef.current);
    questionTimeoutRef.current = setTimeout(() => {
      if (socketRef.current) {
        socketRef.current.emit("question-change", { roomId, question: newQ });
      }
      API.put(`/rooms/save-question/${roomId}`, { examQuestion: newQ })
        .then(() => {
          setQuestionSaveStatus('Saved ✅');
          setTimeout(() => setQuestionSaveStatus(''), 2000);
        })
        .catch(err => {
  ...
  const handleQuestionBlur = () => {
    API.put(`/rooms/save-question/${roomId}`, { examQuestion })
      .then(() => setQuestionSaveStatus('Saved ✅'))
      .catch(() => setQuestionSaveStatus('Failed ❌'));
    setTimeout(() => setQuestionSaveStatus(''), 2000);
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.main}>
        ...
        <div style={styles.editorPanel}>
          ...
          {isOwner && roomType === 'teacher' && (
            <div style={questionExpanded ? styles.questionPanelExpanded : styles.questionPanelCollapsed}>
              <div style={styles.questionHeader}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>📝</span>
                  <span style={{ fontWeight: 'bold', fontSize: '14px' }}>Exam Question</span>
                  {questionSaveStatus && <span style={styles.saveStatus}>{questionSaveStatus}</span>}
                </div>
                <button 
                  onClick={() => setQuestionExpanded(!questionExpanded)} 
                  style={styles.expandBtn}
                >
                  {questionExpanded ? 'Minimize ↙️' : 'Enlarge ↗️'}
                </button>
              </div>
              <textarea
                ...
                placeholder="Write the exam question here... (Students will see this at the top of their screen)"
              />
            </div>
          )}
